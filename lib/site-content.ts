export const siteContent = {
  ...
  projects: [
    ...
    },
    {
      ...
      details: "It is a LangGraph-orchestrated agentic pipeline that takes any tabular dataset and a natural language prompt as input and produces a complete data analysis report without manual configuration. An LLM acts as a decision-maker at each stage choosing the right cleaning strategy, analytics actions, chart types, and insight narrative for the specific dataset and question. The profiler assigns semantic roles (measures, dimensions, datetimes, identifiers) to every column in a single pass, and all downstream stages read from those roles rather than hardcoded column names, making the system fully generalizable across domains. Deterministic Python handles all statistical computation correlations, IQR outlier detection, KMeans clustering, IsolationForest anomaly detection, and RandomForest regression while the LLM is used exclusively for routing decisions and writing Insights, ensuring results are numerically exact and reproducible."
    }
  ],
  skillGroups: [
    {
